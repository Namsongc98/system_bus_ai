#!/usr/bin/env python3
"""Project-local Codex hook policy for System_bus.
...
def resolve_project_root() -> Path:
    ...
    for candidate in candidates:
        if candidate.name == "System_bus" and (candidate / ".codex").exists():
            return candidate
    for candidate in candidates:
        if (candidate / ".codex").exists() and (candidate / "ticket-system").exists() and (candidate / "booking_ticket_vue").exists():
            return candidate
    for candidate in candidates:
        if (
            ...
            and (
                (candidate / "ticket-system").exists()
                or (candidate / "booking_ticket_vue").exists()
            )
        ):
            return candidate
    return current
