> ⚠️ **Đã superseded** — file này mô tả sai gần như toàn bộ API thật (envelope field `code` thay vì
> `status`, path số nhiều cho hầu hết domain trong khi backend dùng số ít, enum Bus status sai...).
> Nguồn thật hiện nay: [`../README.md`](../README.md) (mục lục 17 màn) và
> [`../_conventions.md`](../_conventions.md) (quy ước chung, có bảng đối chiếu path đầy đủ). Giữ file
> này lại chỉ để tham khảo lịch sử — không dùng để code mới.
