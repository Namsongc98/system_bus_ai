...
**Ví dụ tham chiếu (đã cập nhật — trỏ về nguồn thật thay vì `api-document.md` đã superseded):**
`GET /api/ticket` (xem [`../api/my-tickets/endpoints.md`](../api/my-tickets/endpoints.md) cho path/response thật của domain Ticket)
...
## ⚠️ Đính chính — 2 điểm không khớp code thật trong tài liệu này

1. **Envelope lỗi backend thật dùng field `status`, không phải `code`** (mục 6 ở trên mô tả sai) —
   xem [`../api/_conventions.md`](../api/_conventions.md#4-response-envelope-backend--fe-đang-tài-liệu-sai-cần-đọc-theo-bản-backend-thật) mục 4.
2. **Response interceptor thật KHÔNG tự bóc `res.data` khi thành công** (chỉ bóc khi lỗi) — trái với
   giả định ngầm trong ví dụ Service ở mục 10 (`return res.data`). Cách bóc response thật khác nhau
   theo từng service — xem [`../api/_conventions.md`](../api/_conventions.md#3-service-layer-hiện-tại-không-đồng-nhất-cách-bóc-response) mục 2-3 trước khi copy nguyên mẫu ở tài liệu này cho service mới.
