---
description: Commit one task as three commits — booking_ticket_vue (after FE unit tests), ticket-system (after BE unit tests), root System_bus docs (no tests) — staging exactly the report's files on a task/<ID>-<slug> branch, message taken from the plan, then hands over to plan-push.
...
- Follow the skill exactly: gate → commit 1 `booking_ticket_vue` (FE unit tests
  ...
  in the report → commit 3 root repo (no tests). A test failure stops that
  commit and every later one. Each commit: branch `task/<ID>-<slug>`, stage only
  the report's files, verify, commit with the plan-derived message.
