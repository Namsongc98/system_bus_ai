---
name: git-worktree
description: Use when Claude needs to preview, create, list, or safely clean up an isolated Git worktree for the System_bus backend or frontend repository.
---
...
```bash
python3 .claude/skills/git-worktree/scripts/worktree_flow.py preview \
  --repo backend --task ticket-validation

python3 .claude/skills/git-worktree/scripts/worktree_flow.py create \
  --approve '<token>'

python3 .claude/skills/git-worktree/scripts/worktree_flow.py list

python3 .claude/skills/git-worktree/scripts/worktree_flow.py cleanup-preview \
  --repo backend --task ticket-validation

python3 .claude/skills/git-worktree/scripts/worktree_flow.py cleanup \
  --approve '<token>'
```
